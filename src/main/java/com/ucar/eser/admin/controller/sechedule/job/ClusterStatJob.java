...
public class ClusterStatJob extends EsBaseCenterJob {
    ...
    public void action(JobExecutionContext context) {
        try {
            ...
            NodesStats nodesStats = new NodesStats.Builder().withJvm()
                    .withOs()
                    .withIndices()
                    .withHttp()
                    .withTransport()
            ...
            asyncService.submitFuture(new CollectionCommonStatHandler(JobKey.buildFutureKey(clusterName, Constant.INDICES, date),
                    clusterName,
                    json,
                    date));
            asyncService.submitFuture(new CollectionIndicesStatHandler(JobKey.buildFutureKey(clusterName, Constant.INDICES, date),
                    clusterName,
                    json,
                    date));
            asyncService.submitFuture(new CollectionHttpStatHandler(JobKey.buildFutureKey(clusterName, Constant.HTTP, date),
                    clusterName,
                    json,
                    date));
            asyncService.submitFuture(new CollectionJVMStatHandler(JobKey.buildFutureKey(clusterName, Constant.JVM_NAME, date),
                    clusterName,
                    json,
                    date));
            asyncService.submitFuture(new CollectionTransportStatHandler(JobKey.buildFutureKey(clusterName, Constant.TRANSPORT, date),
                    clusterName,
                    json,
                    date));
            asyncService.submitFuture(new CollectionThreadPoolStatHandler(JobKey.buildFutureKey(clusterName, Constant.THREAD_POOL, date),
                    clusterName,
                    json,
                    date));
            ...
            asyncService.submitFuture(new ClusterNodeAlarm(JobKey.buildFutureKey(clusterName, Constant.NODE_ALARM, date),
                    clusterName,
                    json,
                    alarmFilterRule));
        } catch (Exception e) {
