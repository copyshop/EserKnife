...
public abstract class AbstractAlarm extends KeyCallable<Boolean> {
                    ...
                    if (alarmRule.getLatitude().equals(AlarmConstant.AlarmLatitudeEnum.JVM.getName())) {
                        if (alarmRule.getLatitudeSub().equals(AlarmConstant.AlarmLatitudeSubSplitEnum.JVM_OLD_MEM_USED.getName())) {
                            sql = "select oldMemUsed/oldMemMax*100 as result  from jvm " +
                                    "where clusterName = " + clusterName + " and host = '" + alarmParameterVo.getHost() + "' and time >='" + starTime
                                    + "' and time <= '" + endTime + "'";
                        ...
                        sql = "select queue from thread_pool where threadType='" + alarmRule.getLatitudeSub() + "' " +
                                "and clusterName = " + clusterName + " and host = '" + alarmParameterVo.getHost() + "' and time >='" + starTime
                                + "' and time <= '" + endTime + "'";
    ...
    private void ruleDetailTwo(
            AlarmParameterVo alarmParameterVo, AlarmRule alarmRule,
    ...
    private void alarmHandle(
            AlarmParameterVo alarmParameterVo,
            AlarmRule alarmRule,
            String clusterName,
            String latitudeSub,
            Integer count,
