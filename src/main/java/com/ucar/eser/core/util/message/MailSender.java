...
public class MailSender {
	...
	public static boolean sendTextMail(MailInfo mailInfo) throws Exception {
		// 需要身份认证，创建一个密码验证器
		MailAuthenticator authenticator = new MailAuthenticator((String) prop.get("mail.user"),
				(String) prop.get("mail.password"));
		...
		try {
			...
			Address from = new InternetAddress((String) prop.get("mail.user"),
					MimeUtility.encodeText(nick, "UTF-8", "B"));
			// 设置邮件消息的发送者
	...
	private static void addAttachFile(MailInfo mailInfo, Multipart multiPart)
